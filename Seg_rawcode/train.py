...
import timeit
from lib.models.modeling.deeplab import DeepLab
from lib.models.unet.unet_model import UNet
...
valloader = DataLoader(val_dataset, 1, shuffle=True,drop_last=True)
...
if modelname == "axialunet":
    ...
    model = lib.models.axialnet.logo(img_size = 256, imgchan = 3)
elif modelname == "deeplab_resnet":
    model = lib.models.modeling.deeplab.DeepLab(backbone='resnet', output_stride=16)
elif modelname == "deeplab_drn":
    model = lib.models.modeling.deeplab.DeepLab(backbone='drn', output_stride=16)
elif modelname == "deeplab_xception":
    model = lib.models.modeling.deeplab.DeepLab(backbone='xception', output_stride=16)
elif modelname == "ori_unet":
    model = lib.models.unet.unet_model.UNet()
...
for epoch in range(args.epochs):
    ...
    for batch_idx, (X_batch, y_batch, *rest) in enumerate(dataloader):        
        ...
        print("GT Max Label: ",torch.max(y_batch))
        print("Result Max Label: ",torch.max(output))

        loss = criterion(output[:,0,:,:], y_batch.float())
        #loss = criterion(output, y_batch.float())

        print('iter [{}/{}], loss:{:.4f}'
              .format(batch_idx, args.epochs, loss))
    ...
    if (epoch % args.save_freq) ==0:
        ...
        for batch_idx, (X_batch, y_batch, *rest) in enumerate(valloader):
            ...
            X_new_batch=torch.cat([X_batch,X_batch],dim=0)

            # start = timeit.default_timer()
            y_out = model(X_new_batch)
            y_out=y_out[0,:,:,:]
