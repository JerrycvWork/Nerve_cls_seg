...
class ImageToImage2D(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        # print(os.path.join(self.input_path, image_filename))
        image = cv2.imread(os.path.join(self.input_path, image_filename),cv2.IMREAD_GRAYSCALE)
        #image=cv2.cvtColor(gray,cv2.COLOR_GRAY2RGB)
        # print(image.shape)
        # read mask image
        mask = cv2.imread(os.path.join(self.output_path, image_filename[: -3] + "png"),cv2.IMREAD_GRAYSCALE)
        ...
        return image, mask/255, image_filename
